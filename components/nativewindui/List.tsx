...
import { Platform, PressableProps, StyleSheet, View, ViewProps, ViewStyle } from 'react-native';
    ...
    <>
        ...
        <TextClassContext.Provider value="font-normal leading-5">
          ...
          <View
            ...
            <View className={cn('flex-1', textContentClassName)}>
              <Text numberOfLines={textNumberOfLines} className={textClassName}>
                {item.title}
              </Text>
              {!!item.subTitle && (
                <Text
      ...
      {Platform.OS !== 'ios' && !isLastInSection && (
        <View className={cn(variant === 'insets' && 'px-4')}>
          <View style={{ height: StyleSheet.hairlineWidth * 1.5 }} className="bg-border" />
        </View>
      )}
    </>
  );
...
const LIST_SECTION: ViewStyle = Platform.select({
  ios: {},
  default: { borderBottomWidth: StyleSheet.hairlineWidth * 1.5 },
});
  ...
  if (sectionHeaderAsGap) {
    return (
      <View
        ...
        className={cn(
          'bg-background',
          variant === 'full-width' || (Platform.OS !== 'ios' && 'border-b border-border'),
          className
        )}
        {...props}
        ref={ref}>
